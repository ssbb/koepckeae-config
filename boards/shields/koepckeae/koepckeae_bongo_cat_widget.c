...
#include "koepckeae_bongo_cat_widget.h"
...
void state_widget_wpm(struct koepckeae_bongo_cat_widget *widget, int wpm) {
  LOG_DBG("anim state %d", current_anim_state);
  if (wpm < CONFIG_KOEPCKEAE_BONGO_CAT_IDLE_LIMIT) {
  ...
  } else if (wpm < CONFIG_KOEPCKEAE_BONGO_CAT_SLOW_LIMIT) {
...
int wpm_status_listener(const zmk_event_t *eh) {
  struct koepckeae_bongo_cat_widget *widget;
...
ZMK_LISTENER(koepckeae_bongo_cat_widget, wpm_status_listener)
ZMK_SUBSCRIPTION(koepckeae_bongo_cat_widget, zmk_wpm_state_changed);
